...
**S1. 학생 실습** — 학급 코드 + 학번 + 이름으로 입장(12장) → 오늘의 활동 열기 → 부품 드래그로 회로 구성 → 코드 작성·실행 → 오류 발생 → 코치에게 "무엇을 시도했는지"와 함께 질문 → 힌트 받고 수정 → 체크포인트 통과 → 한 줄 성찰 후 제출.
...
모든 코칭 대화는 교사가 열람하며, 학생에게도 첫 사용 시 이를 알린다. 코치는 학생 이름·학번 없이 가명 ID(studentId, 12장 참고)만 받는다.
...
| 영역 | 요구사항 |
| --- | --- |
| 개인정보 | 학생은 학번(studentNo)·이름(name)을 필수로 제출하며, 교사의 학습관리 목적으로 플랫폼에 저장한다. 내부 시스템 관계와 AI 전달에는 studentNo/name 대신 가명 식별자 studentId를 쓴다(12장). 만 14세 미만 학생 정보 수집 시 법정대리인 동의 절차 필요 여부는 확인 필요 |
| 학교 도입 절차 | 학습지원 소프트웨어 선정 기준 검토, 필요 시 학교운영위원회 심의 |
| 데이터 보존 | 학년도 종료 후 일정 기간 뒤 일괄 파기, 교사가 학생 기록 삭제 가능. 구체적 보관 기간·절차는 정책 결정 필요(12장 열린 질문) |
| AI 데이터 전송 | 외부 AI에는 가명 ID(studentId)와 학습 맥락만 전송, 학습 데이터로 쓰이지 않는 API 조건 사용 |
...
| 전달 가능 | 전달하지 않음 |
| --- | --- |
| 가명화된 studentId | 학생 이름(name) |
| 활동(activity) 맥락 | 학번(studentNo) |
| 코드, 회로 상태 | 학급 코드(classCode) |
| checkpoint 상태 | |
| 학생 질문 | |
| 필요한 학습 history | |
